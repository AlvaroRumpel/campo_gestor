...
- **Bypass paralelo em `lots.paddock_id` (MOV-02) — FECHADO no gap cycle #3 (plan 04-07, decisão do usuário 2026-07-16, reversão do escopo travado anterior).** A policy `veterinarian_can_update_active_lot` (`20260514_03_lots_animals.sql:40-50`) nunca inspeciona `paddock_id` no `WITH CHECK`, então um veterinário membro de duas propriedades podia contornar `move_lot_to_paddock` com um `PATCH /lots {paddock_id: <piquete de outra propriedade>}` bruto. Fechado pelo trigger `BEFORE INSERT OR UPDATE` `trg_lots_paddock_same_property` em `lots` (migration `supabase/migrations/20260717_04_lot_paddock_property_trigger.sql`), validando `paddock_id ∈ property_id` — espelhando o `trg_animals_lot_same_property` do gap cycle #2 (SC-4/animais). Ver 04-REVIEW.md WR-02 / CR-01-parallel para o achado original; T-4-08 no threat register de 04-07-PLAN.md move de `accept` para `mitigate`.
