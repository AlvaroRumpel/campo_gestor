...
part 'iatf_model.freezed.dart';
part 'iatf_model.g.dart';

@freezed
sealed class IatfBatch with _$IatfBatch {
  // ignore: invalid_annotation_target
  @JsonSerializable(fieldRename: FieldRename.snake)
  const factory IatfBatch({
    required String id,
  ...
  factory IatfBatch.fromJson(Map<String, dynamic> json) =>
      _$IatfBatchFromJson(json);
}

/// A row from `animal_iatf_memberships` joined with
...
class IatfMembershipView {
  const IatfMembershipView({
    required this.membershipId,
    required this.iatfBatchId,
  ...
  /// to the membership merely being deactivated because the IATF was closed
...
///
/// One row per IATF the animal participated in — active or closed memberships
/// alike — carrying that IATF's most recent DG result. Built from
/// `animal_iatf_memberships` joined with `iatf_batches(id, name,
...
class ReproductiveHistoryEntry {
  ...
  final bool iatfActive;

  /// Null means "aguardando DG" — the animal is in this IATF but has no DG yet.
  ...
  /// All DGs for this animal in this IATF, ordered most-recent-first per the
  /// single [isLaterDg] rule (D-09, D-10). Empty (never null) when the IATF
  /// has no DG for this animal yet.
  final List<DgRecord> dgRecords;

  /// Bull label already resolved on the IATF batch (Phase 5, WR-01); null when
  /// the IATF has no named bull.
