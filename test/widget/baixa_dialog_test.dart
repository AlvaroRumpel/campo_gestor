...
// effects on the IATF composition provider family are observable via a
// build counter.
// ---------------------------------------------------------------------------
...
void main() {
  ...
  group('G-05-1: BaixaDialog invalidates IATF composition providers', () {
    testWidgets(
        'a successful baixa rebuilds iatfActiveMembershipsProvider, '
        'iatfMembershipsProvider, and iatfListByPropertyProvider once each',
        (tester) async {
      var activeBuilds = 0;
      var allBuilds = 0;
      var listBuilds = 0;
      ...
      final container = ProviderContainer(
        overrides: [
          ...
          animalListByPropertyProvider.overrideWith((ref) async => []),
          animalByIdProvider.overrideWith((ref, id) async => null),
          iatfActiveMembershipsProvider.overrideWith((ref, id) async {
            activeBuilds++;
            return const <IatfMembershipView>[];
          }),
          iatfMembershipsProvider.overrideWith((ref, id) async {
            allBuilds++;
            return const <IatfMembershipView>[];
          }),
          iatfListByPropertyProvider.overrideWith((ref) async {
            listBuilds++;
            return const <IatfSummary>[];
          }),
        ],
      );
      ...
      container.listen(
        iatfActiveMembershipsProvider('iatf-1'),
        (_, _) {},
        fireImmediately: true,
      );
      container.listen(
        iatfMembershipsProvider('iatf-1'),
        (_, _) {},
        fireImmediately: true,
      );
      container.listen(
        iatfListByPropertyProvider,
        (_, _) {},
        fireImmediately: true,
      );
      ...
      expect(activeBuilds, 2,
          reason: 'iatfActiveMembershipsProvider must rebuild once post-baixa');
      expect(allBuilds, 2,
          reason: 'iatfMembershipsProvider must rebuild once post-baixa');
      expect(listBuilds, 2,
          reason: 'iatfListByPropertyProvider must rebuild once post-baixa');
    });
  });
}
