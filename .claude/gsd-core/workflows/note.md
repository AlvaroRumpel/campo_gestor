...
- **Global scope**: `F:/_geral/Projetos/campo_gestor/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/_geral/Projetos/campo_gestor/.claude/notes/`)
...
2. Glob `F:/_geral/Projetos/campo_gestor/.claude/notes/*.md` (if directory exists) — global notes
...
Global (F:/_geral/Projetos/campo_gestor/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/_geral/Projetos/campo_gestor/.claude/notes/` — works in any directory
